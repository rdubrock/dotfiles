---
name: babysit-ci
description: Babysit CI for the current branch PR until all checks pass. Monitors check status, fixes failures, addresses AI review comments, and pushes fixes incrementally. Use after opening a PR or when CI needs attention.
---
...
6. Run local-first quality gates before entering CI watch mode (target changed files when possible):
   - For frontend (if `package.json` exists):
     - `yarn lint:fix` (ESLint + Prettier formatting)
     - `yarn typecheck` (TypeScript type checking)
   - For Go (if `go.mod` exists):
     - `gofmt -w .` (format Go files)
     - `golangci-lint run --fix` (lint and auto-fix Go code)
